...
class AerialVG(nn.Module):
    def __init__(
        ...
        self.sim_param = torch.tensor(0.3)
        # 上采样卷积层（用于扩大上层特征图）
        ...
        # 下采样卷积层（用于缩小特征图）
        self.downsample_conv = self.downsample_conv = SharedKernelConv(kernel_size=3)
    ...
    def forward(self, samples: NestedTensor, targets: List = None, **kw):
            ...
            # 将加权后的 logits 组合起来
            outputs_class[-1] = weighted_original_logits.scatter_add(1, expanded_topk_indices, weighted_new_logits)
            # print(' outputs_class[-1]', outputs_class[-1].shape, outputs_class[-1])

            # # outputs_class[-1] = final_pred_logits.scatter_add(1, expanded_topk_indices, new_logits)


            out = {"pred_logits": outputs_class[-1], "pred_boxes": outputs_coord_list[-1]} #(1,900,256) (,1,900,4)
