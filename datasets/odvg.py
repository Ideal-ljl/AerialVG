...
if __name__=="__main__":
    ...
    dataset_vg = ODVGDataset("/cpfs01/shared/optimal/liujunli/UAVVG/dataset/vg/images","/cpfs01/shared/optimal/liujunli/UAVVG/dataset/vg/annotation/vg_train_odvg.jsonl",None, transforms=make_coco_transforms('test'))
    # print(len(dataset_vg))
    # print(dataset_vg[0])
    # exit()
    sampler_val = torch.utils.data.SequentialSampler(dataset_vg)
    data_loader_val = DataLoader(dataset_vg, 8,sampler_val,collate_fn=utils.collate_fn,drop_last=False)
    for samples, targets in data_loader_val:
        captions = [t["anno"] for t in targets]
        cap_list = [t["cap_list"][0] for t in targets]
        device = 'cuda'
